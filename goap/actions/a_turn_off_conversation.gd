...
func setup():
	...
	name = 'a_turn_off_conversation'               # String identifier of this action.  setup_common() will make sure the action's node is named after this
	...
	add_effect('s_disable_conversation', true)
	return
...
func execute():
	...
	if agent.goals_current[0][0].get('currentplyr'):
		var curplyr = agent.goals_current[0][0].get('currentplyr')
		var angleplyr = agent.goals_current[0][0].get('angletoplyr')
		entity.disable_conversation(curplyr, angleplyr)
	return COMPLETED
