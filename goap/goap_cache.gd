...
const UPDATE_INTERVAL_DEFAULT = 0.1           # Time in seconds between an agent's execute() calls
...
# cache_actions_from_type - a array with the names of actions sorted by type (NORMAL, REPEATABLE, etc.)
...
func _ready():
	...
	if (dir.open("res://goap/actions") == OK):
		...
		while filename != '':
			if filename.begins_with('a_') and filename.ends_with('.tscn'):
				filename = filename.get_basename()
				cache_actions[filename] = "res://goap/actions/" + filename + ".tscn"
				if debug_show_cache_actions:
					print('GOAP PRELOAD: Added action ' + str(filename))
			filename = dir.get_next()
		dir.list_dir_end()
	else:
		print('GOAP FATAL ERROR:  CACHE UNABLE TO LOCATE ACTIONS DIRECTORY')
	
	if (dir.open("res://goap/goals") == OK):
		dir.list_dir_begin()
		var filename = dir.get_next()
		while filename != '':
			if filename.begins_with('g_') and filename.ends_with('.tscn'):
				filename = filename.get_basename()
				cache_goals[filename] = "res://goap/goals/" + filename + ".tscn"
				if debug_show_cache_actions:
					print('GOAP PRELOAD: Added goal ' + str(filename))
			filename = dir.get_next()
		dir.list_dir_end()
	else:
		print('GOAP FATAL ERROR:  CACHE UNABLE TO LOCATE GOALS DIRECTORY')
...
func setup():
	...
	for goal_name in cache_goals.keys():
		...
		var goal = load(cache_goals[goal_name]).instance()
		add_child(goal)
		goal.setup()
		
		var symbols = goal.get_symbols()
		for symbol in symbols:
			
			if !cache_symbols.has(symbol):
				# Symbol hasn't been added yet, so do that
				cache_symbols.append(symbol)
				if debug_show_cache_symbols:
					print('GOAP PRELOAD: Added "' + str(symbol) + '" to symbols')
			
			if !cache_goals_from_symbol.has(symbol):
				cache_goals_from_symbol[symbol] = []
			cache_goals_from_symbol[symbol].append(goal_name)
			
			if debug_show_cache_goals_from_symbol:
				print('GOAP PRELOAD: Added "' + str(goal_name) + '" to cache_goals_from_symbol[' + symbol + ']')
			...
			if !cache_goals_from_symbol_value.has(valuekey):
				...
				if debug_show_cache_goals_from_symbol_value:
					print('GOAP PRELOAD: Added "' + str(valuekey) + '" as key to cache_goals_from_symbol_value')
			
			cache_goals_from_symbol_value[valuekey].append(goal_name)
			if debug_show_cache_goals_from_symbol_value:
				print('GOAP PRELOAD: Added "' + str(goal_name) + '" to cache_goals_from_symbol_value[' + valuekey + ']')
	...
	for action_name in cache_actions.keys():
		...
		var action = load(cache_actions[action_name]).instance()
		add_child(action)
		action.setup()
		...
		for symbol in symbols.keys():
			...
			if !cache_symbols.has(symbol):
				cache_symbols.append(symbol)
				if debug_show_cache_symbols: 
					print("GOAP PRELOAD: Added '" + symbol + "' to symbols")
			
			if !cache_actions_from_effect.has(symbol):
				cache_actions_from_effect[symbol] = []
				if debug_show_cache_actions_from_effect: 
					print("GOAP PRELOAD: Added '" + symbol + "' as key to cache_actions_from_effect")
			
			cache_actions_from_effect[symbol].append(action_name)
			if debug_show_cache_actions_from_effect: 
				print("GOAP PRELOAD: Added '" + action_name + "' to cache_actions_from_effect[" + symbol + "]")
			...
			if !cache_actions_from_effect_value.has(valuekey):
				...
				if debug_show_cache_actions_from_effect_value: 
					print("GOAP PRELOAD: Added '" + valuekey + "' as key to cache_actions_from_effect_value")
			
			cache_actions_from_effect_value[valuekey].append(action_name)
			if debug_show_cache_actions_from_effect_value: 
				print("GOAP PRELOAD: Added '" + action_name + "' to cache_actions_from_effect_value[" + valuekey + "]")
		...
		for symbol in symbols.keys():
			...
			if !cache_actions_from_precondition.has(symbol):
				...
				if debug_show_cache_actions_from_precondition: 
					print("GOAP PRELOAD: Added '" + symbol + "' as key to cache_actions_from_precondition")
			
			cache_actions_from_precondition[symbol].append(action_name)
			if debug_show_cache_actions_from_precondition: 
				print("GOAP PRELOAD: Added '" + action_name + "' to cache_actions_from_precondition[" + symbol + "]")
			
			var valuekey = symbol + "|" + str(symbols[symbol])
			
			if !cache_actions_from_precondition_value.has(valuekey):
				cache_actions_from_precondition_value[valuekey] = []
				if debug_show_cache_actions_from_precondition_value: 
					print("GOAP PRELOAD: Added '" + valuekey + "' as key to cache_actions_from_precondition_value")
			
			cache_actions_from_precondition_value[valuekey].append(action_name)
			if debug_show_cache_actions_from_precondition_value: 
				print("GOAP PRELOAD: Added '" + action_name + "' to cache_actions_from_precondition_value[" + valuekey + "]")
		...
		if action.type > cache_actions_from_type.size() - 1:
			...
			while i < cache_actions_from_type.size():
				if cache_actions_from_type[i] == null: 
					cache_actions_from_type[i] = []
				i += 1
		
		cache_actions_from_type[action.type].append(action_name)
		if debug_show_cache_actions_from_type: 
			print("GOAP PRELOAD: Added '" + action_name + "' to cache_actions_from_type[" + str(action.type) + "]")
		action.queue_free()
...
func printout():
	...
	if debug_show_cache_actions: 
		print("GOAP PRELOAD READOUT: cache_actions = " + str(cache_actions))
	if debug_show_cache_actions_from_effect: 
		print("GOAP PRELOAD READOUT: cache_actions_from_effect = " + str(cache_actions_from_effect)) 
	if debug_show_cache_actions_from_effect_value: 
		print("GOAP PRELOAD READOUT: cache_actions_from_effect_value = " + str(cache_actions_from_effect_value)) 
	if debug_show_cache_actions_from_precondition: 
		print("GOAP PRELOAD READOUT: cache_actions_from_precondition = " + str(cache_actions_from_precondition)) 
	if debug_show_cache_actions_from_precondition_value: 
		print("GOAP PRELOAD READOUT: cache_actions_from_precondition_value = " + str(cache_actions_from_precondition_value))
	if debug_show_cache_actions_from_type: 
		print("GOAP PRELOAD READOUT: cache_actions_from_type = " + str(cache_actions_from_type))
	
	if debug_show_cache_goals: 
		print("GOAP PRELOAD READOUT: cache_goals = " + str(cache_goals))
	if debug_show_cache_goals_from_symbol: 
		print("GOAP PRELOAD READOUT: cache_goals_from_symbol = " + str(cache_goals_from_symbol))
	if debug_show_cache_goals_from_symbol_value: 
		print("GOAP PRELOAD READOUT: cache_goals_from_symbol_value = " + str(cache_goals_from_symbol_value))
	
	if debug_show_cache_symbols: 
		print("GOAP PRELOAD READOUT: cache_symbols: " + str(cache_symbols))
