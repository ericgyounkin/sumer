...
func setup_common():
	...
	if debug_show_call_setup_common:
		print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '" CALLED setup_common')
	...
	if actions_available.size() < 1:
		print('GOAP AGENT ' + str(entity.name) + ' HAS NO ACTIONS AVAILABLE!')
	...
	if goals_available.size() < 1:
		print('GOAP AGENT ' + entity.name + ' HAS NO GOALS AVAILABLE!')
	...
	updater = Timer.new()
	add_child(updater)
	updater.wait_time = update_interval
...
func no_goals():
	print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '": NO GOALS!')

func plan_aborted(action_name):
	print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '": PLAN ABORTED! CURRENT ACTION: ' + action_name)
	
func plan_failed():
	print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '": PLAN FAILED!')
...
func plan_found(cost):
	...
	while i < actions_current.size():
		...
		i += 1
	plan_string += '(End Of Plan)'
	print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '": PLAN FOUND! COST: ' + str(cost) + ' PLAN: ' + plan_string)
	
func generate_plan():
	if debug_show_call_generate_plan:
		print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '" CALLED generate_plan()')
	...
	while goals_current.size() > 0:
		...
		# Plan returns its cost as elelment [0]
...
func generate_goalqueue():
	if debug_show_call_generate_goalqueue:
		print(str(OS.get_ticks_msec()) + ': GOAP AGENT "' + entity.name + '" CALLED generate_goalqueue()')
	...
	if (goals_available.size() > 0):
		...
		if (debug_show_call_generate_goalqueue): 
			...
			while (i < goals_current.size()):
				...
				i += 1
			goal_string += "(End Of Queue)"
			print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.name + "' generate_goalqueue() RETURNING " + goal_string)
		...
		print(str(OS.get_ticks_msec()) + ": GOAP_AGENT ENTITY " + entity.name + " HAS NO GOALS AVAILABLE!")
		return


func plan(state_goal, state_world):
	
	if debug_show_call_plan: 
		print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.name + "' CALLED plan(GOALSTATE: "\
...
func build_graph(action_inspected, worldstate, goalstate, action_pool, list_closed):
	...
	if debug_show_call_build_graph:
		...
		print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.name + "' CALLED build_graph(ACTION_INSPECTED: " + action_name + "; WORLDSTATE: " + str(worldstate) + "; GOALSTATE: " + str(goalstate)\
	...
	if compare_states(goalstate, worldstate):
		...
		while (actions_selectable.get_size() > 0):
			...
			if (debug_show_call_build_graph):
				...
				print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.name + "': NESTED build_graph() FOR ACTION '" + action.name + "' RETURNED: " + str(path_string))
...
func interval():
	...
	if (debug_show_call_interval): print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.name + "' CALLED interval()")
...
func execute():
	...
	if (debug_show_call_execute): print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.name + "' CALLED execute()")
	...
	if (status == ACTION_CONTINUED):
		...
		print(str(OS.get_ticks_msec()) + ": GOAP AGENT '" + entity.get_name() + "' RECEIVED UNEXPECTED VALUE '" + str(status) + "' FROM ACTION '" + actions_current[0].get_name() + "'")
		return
...
# Copied and modified by specific agent
