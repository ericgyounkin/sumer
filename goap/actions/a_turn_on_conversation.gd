...
func setup():
	...
	name = 'a_turn_on_conversation'               # String identifier of this action.  setup_common() will make sure the action's node is named after this
	...
	add_effect('s_enable_conversation', true)
	return
...
func execute():
	...
	if agent.goals_current[0][0].get('currentplyr'):
		...
		entity.enable_conversation(curplyr, angleplyr)
	return COMPLETED
