...
func setup():
	...
	name = 'a_move_normal'               # String identifier of this action.  setup_common() will make sure the action's node is named after this
...
func execute():
	...
	if entity.is_network_master():
		if !is_processing():
			...
			if entity.running:
				...
				entity.move_speed = entity.walk_speed * global.walk_speed_modifier
			entity.rset('slave_move_speed', entity.move_speed)
			if global.netdebug:
				print('rset slave_moving slave_move_speed')
			entity.update_navigation_path(entity.global_position, entity.mousepos)
			set_process(true)
	
	if entity.fleeing or entity.check_having_conversation():
		entity.moving = false
		entity.rset('slave_moving', entity.moving)
		if global.netdebug:
	...
	elif entity.moving == false:
		if agent.actions_current[1].get('pause'):       # Wander/approach gets you here
			if $wander_timer.is_stopped():
				$wander_timer.wait_time = rand_range(2,4) * agent.actions_current[1].get('pause')
				$wander_timer.start()
			if finished:
				finished = false
				set_process(false)
