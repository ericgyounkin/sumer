...
const TYPE_REPEATABLE = 2          # This goal / symbol may be repeatably executed
...
var goals = {}                     # Dictionary of all possible goalstates to be returned with priorities and type
...
var type = TYPE_NORMAL             # Defines goal type.  See constants
...
func setup_common():
	...
	# Set priority for diminishing type goals 
...
# Copied and modified by specific goals

func setup():
	# Called after setup_common()
	
	name = ''           # String to identify the goal from others after creation
	priority = 1.0      # The higher the number, the more important the goal.  Agent will always want to fulfill the goal with the highest priority first
...
func evaluate():
	...
	# If one sumbol is not fulfilled this goal may be used in planning
	# Returns a dictionary of all goal's symbols in worldstate if any is not satisfied, otherwise null
...
func on_completion():
	...
	# DIMINISHING type goals will reduce their priority automatically to priority_default / times_completed + 1
